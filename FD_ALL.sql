...
-- 기존 테이블 생성했던 사람은 DROP 해주고 다시 생성
...
-- 기존 TABLE 생성한 사람은 시퀸스도 지우고 다시 생성하기
...
INSERT INTO CARRYFUNDING_PROJECT VALUES(
    SEQ_PROJECT_NO.NEXTVAL,'프로젝트TEST','CARRY',1000000,500000,DEFAULT,DEFAULT,'21/01/25',
    '이미지원래이름.JPG','이미지변경이름.JPG','프로젝트 TEST 입니다. 무슨 내용을 넣어야 할까요? 과연 엔터도 먹을까요?
    줄바꿈 같은것도 직접해야할까? 고민이네요ㅋㅋㅋ','Y',DEFAULT,DEFAULT,DEFAULT,1
);
...
INSERT INTO CARRYFUNDING_PROJECT VALUES(
    SEQ_PROJECT_NO.NEXTVAL,'조회TEST용','CARRY',1000000,200000,DEFAULT,DEFAULT,'21/05/25',
    '이미지이름.JPG','이미지이름.JPG','프로젝트 TEST 입니다. 무슨 내용을 넣어야 할까요? 과연 엔터도 먹을까요?
    줄바꿈 같은것도 직접해야할까? 고민이네요ㅋㅋㅋ','Y',DEFAULT,DEFAULT,DEFAULT,1
);

INSERT INTO CARRYFUNDING_PROJECT VALUES(
    SEQ_PROJECT_NO.NEXTVAL,'조회TEST용','CARRY',1000000,300000,DEFAULT,DEFAULT,'21/06/25',
    '이미지이름.JPG','이미지이름.JPG','프로젝트 TEST 입니다. 무슨 내용을 넣어야 할까요? 과연 엔터도 먹을까요?
    줄바꿈 같은것도 직접해야할까? 고민이네요ㅋㅋㅋ','Y',DEFAULT,DEFAULT,DEFAULT,1
);

INSERT INTO CARRYFUNDING_PROJECT VALUES(
    SEQ_PROJECT_NO.NEXTVAL,'조회TEST용','CARRY',1000000,100000,DEFAULT,DEFAULT,'21/07/25',
    '이미지이름.JPG','이미지이름.JPG','프로젝트 TEST 입니다. 무슨 내용을 넣어야 할까요? 과연 엔터도 먹을까요?
    줄바꿈 같은것도 직접해야할까? 고민이네요ㅋㅋㅋ','Y',DEFAULT,DEFAULT,DEFAULT,1
);
...
-- 프로젝트 달성률 내림차순으로 조회 + 페이징처리까지 WHERE절 BETWEEN 다음 숫자값을 ?로 주고 JAVA에서 처리하기
SELECT *
FROM (
...
-- 프로젝트 게시글 전체 수 조회하기
SELECT COUNT(*) FROM CARRYFUNDING_PROJECT WHERE PROJECT_STATUS = 'Y' AND PROJECT_CHECK = 'Y';
...
INSERT INTO BOARD VALUES(SEQ_BOARD_NO.NEXTVAL, 25, '게시글 1',  '우하하하하...', '원본파일명.txt', '변경된파일명.txt', DEFAULT, 'Y', SYSDATE, SYSDATE);
...
COMMIT;
-- POINTUPDATE 부분에서 사용한 쿼리문
--update member set user_coin = (user_coin + ?) where user_no = ?;
